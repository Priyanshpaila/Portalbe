...
/** ------------------ DEBUG LOG ------------------ **/
const DEBUG_PO = String(process.env.DEBUG_PO || "").toLowerCase() === "true";
const dlog = (...args) => {
  if (DEBUG_PO) console.log("[PO]", ...args);
};
/** ----------------------------------------------- **/

/** ------------------ SAP PO NUMBER GENERATION ------------------ **/
const SAP_PO_PREFIX = "SAP";
const SAP_PO_TOTAL_LEN = 16;
const SAP_PO_DIGITS = SAP_PO_TOTAL_LEN - SAP_PO_PREFIX.length; // 13
const SAP_PO_COUNTER_ID = "SAP_PO"; // will be stored in counter._id

const getNextSapPONumber = async () => {
  const doc = await counterModel.findOneAndUpdate(
    { _id: SAP_PO_COUNTER_ID },          // ✅ USE _id (matches your schema)
    { $inc: { seq: 1 } },
    { new: true, upsert: true, setDefaultsOnInsert: true }
  );

  const num = String(doc.seq).padStart(SAP_PO_DIGITS, "0");
  const sapPONumber = `${SAP_PO_PREFIX}${num}`;
  dlog("Generated sapPONumber:", sapPONumber, "seq:", doc.seq);
...
const syncCSQtyHandler = (po) =>
  syncCSQuantity([po.refCSNumber, ...(po.items || []).map((i) => i.csNumber)].filter(Boolean));
...
poRouter.post(
  ...
  async (req, res, next) => {
    try {
      dlog("POST /po create. user:", req.user?._id, "newId:", req.params.id);

      const data = JSON.parse(req.body.data || "{}");
      dlog("Incoming PO keys:", Object.keys(data || {}));

      if (data?.readyForAuthorization) {
        if (!data.authorize?.length) throw createError("At least one authorization level is required.", 400);
        if (!data.items?.length) throw createError("At least one item is required.", 400);
      }

      // ✅ AUTO-GENERATE sapPONumber
      if (!data.sapPONumber || !String(data.sapPONumber).trim()) {
        data.sapPONumber = await getNextSapPONumber();
      }
      dlog("Final sapPONumber:", data.sapPONumber);

      const created = await poModel.create({
        ...data,
        _id: req.params.id,
        createdBy: req.user?._id,
      });

      dlog("PO created:", created?._id, created?.poNumber, created?.sapPONumber);
      ...
      let errorMessage;
      try {
        await mailPO(created.toObject());
      } catch (err) {
        console.error("PO created but mail failed:", err);
        errorMessage = "PO created but failed to send email to user";
      }

      res.status(201).json({ success: true, errorMessage });
    } catch (error) {
      console.error("Error in PO create:", error);
      next(error);
    }
  }
);
...
poRouter.put("/:id", upload.array("file"), async (req, res, next) => {
  try {
    ...
    dlog("PUT /po update. id:", id, "user:", req.user?._id);
    ...
    } else if (data?.authorize && !data.authorize.some((i) => i.approvalStatus !== 1)) {
      const po = await poModel.findById(id, { amendNumber: 1 });
      data.amendNumber = (po?.amendNumber || 0) + 1;
      data.amendHistory = [
        { amendedBy: req.user._id, amendedAt: Date.now() },
        ...(data.amendHistory || []),
      ];
    ...
    const updated = await poModel.findByIdAndUpdate(
      id,
      { ...data, updatedBy: req.user?._id },
      { new: true }
    );

    dlog("PO updated:", updated?._id, updated?.sapPONumber);
    ...
    let errorMessage;
    try {
      await mailPO(updated?.toObject?.() ?? updated);
    } catch (error) {
      console.error("PO updated but mail failed:", error);
      errorMessage = "PO updated but failed to send email to user";
    }

    res.status(200).json({ ...(updated?.toObject?.() ?? updated), errorMessage });
  } catch (error) {
    console.error("Error in PO update:", error);
    next(error);
  }
});
  ...
  try {
    ...
    if (req.user.vendorCode) {
      ...
      pipeline = [
        ...
        {
          $replaceRoot: {
            newRoot: { $mergeObjects: ["$doc", { authorize: "$authorize" }] },
          },
        },
        {
          $set: {
            lastAuthorize: {
              $last: {
                $sortArray: { input: "$authorize", sortBy: { changedOn: 1 } },
              },
            },
          },
        },
    ...
    if (filters) {
      ...
      if (!req.user.vendorCode) {
        ...
        if (filters.status === "initial") {
          filter.authorize = { $elemMatch: { $or: [{ approvalStatus: 0 }, { approvalStatus: 2 }] } };
        }
      ...
      if (filters.poDate?.[0]) {
        filter.poDate = {};
        if (filters.poDate[0]) filter.poDate["$gte"] = new Date(new Date(filters.poDate[0]).setHours(0, 0, 0, 0));
        if (filters.poDate[1]) filter.poDate["$lte"] = new Date(new Date(filters.poDate[1]).setHours(24, 0, 0, 0) - 1);
      }
...
poRouter.get("/", async (req, res, next) => {
  try {
    ...
    const po =
      (id?.length === 22 ? await poModel.findById(id) : await poModel.findOne({ poNumber }))?.toJSON();

    if (!po) return res.status(404).send(null);
