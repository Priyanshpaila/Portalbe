...
const updateQuotations = async (data) => {
  ...
  await Promise.all([
    quotationModel.updateMany(
      { quotationNumber: { $in: selectedQuotations } },
      { status: 1.5 },
    ),
    quotationModel.updateMany(
      {
...
async function deriveCompanyFromItems(items = []) {
  try {
    const pairs = (items || [])
      .map((i) => ({
      ...
      .filter((x) => x.indentNumber && x.itemCode);
    ...
    // ✅ assuming CS belongs to single company
    return companies[0] || "";
  } catch (e) {
    console.error("[deriveCompanyFromItems] failed:", e);
    return "";
  }
}
...
csRouter.get("/generate", async (req, res, next) => {
  try {
    ...
    const rfq = await RFQModel.findOne(
      { rfqNumber },
      { rfqDate: 1, items: 1, vendors: 1, dueDate: 1 },
    );
    ...
    if (!quotations?.length)
      throw createError("No quotations found for this RFQ", 405);
        ...
        return {
          ...
          total: q
            ? {
                ...
                gst: +(q.amount.igst || q.amount.cgst + q.amount.sgst).toFixed(
                  2,
                ),
      ...
      items: rfq.items.map((i) => ({
        ...
        vendors: rfq?.vendors?.map((rfqV) => {
          const qItem = quotations
            ...
            ?.items.find(
              (_i) =>
                _i.itemCode === i.itemCode &&
                _i.indentNumber === i.indentNumber,
            );
    ...
    for (let i = 0; i < csDoc.vendors.length; i++) {
      ...
      if (
        total.basicAfterDiscount < csDoc.leastValues.basicAfterDiscount.value
      ) {
    ...
    csDoc.company = await deriveCompanyFromItems(csDoc.items);
...
csRouter.post("/list", async (req, res, next) => {
  try {
    ...
    if (filters) {
      ...
      if (filters.company)
        filter.company = { $regex: filters.company.trim(), $options: "i" };
      if (filters.csNumber) filter.csNumber = filters.csNumber.trim();
      if (filters.csDate?.[0]) {
        filter.csDate = {};
        if (filters.csDate[0])
          filter.csDate["$gte"] = new Date(
            new Date(filters.csDate[0]).setHours(0, 0, 0, 0),
          );
        if (filters.csDate[1])
          filter.csDate["$lte"] = new Date(
            new Date(filters.csDate[1]).setHours(24, 0, 0, 0) - 1,
          );
      }
      if (filters.rfqNumber) filter.rfqNumber = filters.rfqNumber.trim();
      if (filters.vendorCode)
        filter["selection.vendorCode"] = filters.vendorCode.trim();
...
csRouter.put(
  "/",
  authorizePermissions(PERMISSIONS.MANAGE_CS),
  async (req, res, next) => {
    try {
      const data = req.body;
      ...
      data.company = await deriveCompanyFromItems(data.items);
      ...
      if (!updatedCS)
        return res
          .status(404)
          .json({ success: false, message: "CS not found" });
...
csRouter.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      if (po)
        throw createError(
          "CS can not be deleted as a PO has been generated for this CS.",
          403,
        );
