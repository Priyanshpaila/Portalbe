...
const mailRFQ = async (data, user) => {
	...
	for (const vendor of data.vendors) {
		let vendorUser = await userModel.findOne({ vendorCode: vendor.vendorCode }, { _id: 1, email: 1 })
	...
	await sendMail(emailRecipients)
}
...
rfqRouter.post(
	...
	async (req, res, next) => {
		try {
			...
			if (data?.attachments?.length) for (const i of data.attachments) i.status = 1

			if (data.status === 1) {
				if (data.vendors?.length === 0) throw createError("At least one vendor is required", 400)
				if (data.items?.length === 0) throw createError("At least one item is required", 400)
...
rfqRouter.put("/dueDate", authorizePermissions(PERMISSIONS.MANAGE_RFQ), async (req, res, next) => {
	try {
...
rfqRouter.get("/resend-email", async (req, res, next) => {
	try {
		const { rfqId, vendorCode } = req.query
		const rfq = await rfqModel.findById(rfqId, { rfqNumber: 1, dueDate: 1, vendors: 1 })
		if (!rfq) throw createError("RFQ not found", 404)

		await mailRFQ({ ...rfq, vendors: rfq.vendors?.filter((i) => i.vendorCode === vendorCode) })
...
rfqRouter.get("/:id", async (req, res, next) => {
	try {
		...
		data.rfq = await rfqModel.findOne(
			id.length === 24 ? { $or: [{ _id: new Types.ObjectId(id) }, { rfqNumber: id }] } : { rfqNumber: id }
		)

		if (!data.rfq) throw createError("RFQ not found", 404)

		if (req.user.vendorCode && req.user?.permissions?.includes(PERMISSIONS.VENDOR_ACCESS))
			data.rfq.vendors = data.rfq.vendors.filter((i) => i.vendorCode === req.user.vendorCode)
...
rfqRouter.put(
	...
	async (req, res, next) => {
		try {
			...
			const updatedRFQ = await rfqModel.findByIdAndUpdate(req.params.id, { $set: data }, { new: true })
			if (!updatedRFQ) throw createError("RFQ not found", 404)

			if (updatedRFQ.items?.length)
				await syncIndentQuantity({
					indents: updatedRFQ.items.map((i) => ({ indentNumber: i.indentNumber, itemCode: i.itemCode })),
					shouldUpdate: true
				})
...
rfqRouter.patch(
	...
	async (req, res, next) => {
		try {
			...
			let errorMessage
			try {
				await mailRFQ({ ...rfq.toObject(), vendors: [vendor], dueDate: rfq.dueDate }, req.user._id)
			} catch (error) {
...
rfqRouter.patch("/regret", authorizeTokens, authorizePermissions(PERMISSIONS.VENDOR_ACCESS), async (req, res, next) => {
	try {
		const { rfqNumber } = req.query
		const rfq = await rfqModel.findOne({ rfqNumber })
		if (!rfq) throw createError("RFQ not found", 404)

		if (!req.user.vendorCode || !rfq.vendors?.some((i) => i.vendorCode === req.user.vendorCode))
...
rfqRouter.delete("/:id", authorizeTokens, authorizePermissions(PERMISSIONS.MANAGE_RFQ), async (req, res, next) => {
	try {
		const rfq = await rfqModel.findById(req.params.id, { rfqNumber: 1, items: 1 })
		if (!rfq) throw createError("Invalid Id, RFQ not found.", 400)

		const recordsExists = await quotationModel.countDocuments({ rfqNumber: rfq?.rfqNumber })
		if (recordsExists) throw createError(recordsExists + " quotations have been submitted for this RFQ.", 400)
		else {
			await rfqModel.findByIdAndDelete(req.params.id)
			await syncIndentQuantity({
				indents: rfq.items.map((i) => ({ indentNumber: i.indentNumber, itemCode: i.itemCode })),
				shouldUpdate: true
			})
